...
to play-n-rounds [ n partner ]
  ...
  repeat n [
    ...
    let my-move move-error make-move strategy (trimmed-history memory-length history)
    ...
    let my-payoff ifelse-value (my-move = 1) [
      ifelse-value (partner-move = 1) [win-win-payout][0]
    ][
      ifelse-value (partner-move = 1) [1][lose-lose-payout]
    ]

    let partner-payoff ifelse-value (partner-move = 1) [
      ifelse-value (my-move = 1) [win-win-payout][0]
    ][
      ifelse-value (my-move = 1) [1][lose-lose-payout]
    ]
...
<experiments>
  ...
  <experiment name="memory cost" repetitions="10" runMetricsEveryStep="false">
    ...
    <timeLimit steps="50"/>
    <metric>ticks</metric>
    <steppedValueSet variable="cost-of-memory" first="0" step="0.05" last="0.5"/>
    <enumeratedValueSet variable="split-finetune">
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="wildcard-finetune">
    ...
    <steppedValueSet variable="cost-of-existence" first="0" step="0.05" last="0.2"/>
    <enumeratedValueSet variable="assume-cooperation?">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="wildcard-p-magnitude">
